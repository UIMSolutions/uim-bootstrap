...
@safe: class DBS5ButtonSubmit : DBS5ButtonObj {
	...
	unittest {
		assert(Assert(BS5ButtonSubmit.active(true),`<button class="active btn" type="submit"></button>`));
	}

	O block(this O)(bool mode = true){ if (mode) this.classes("btn-block"); return cast(O)this; }
	unittest {
		assert(Assert(BS5ButtonSubmit.block(true),`<button class="btn btn-block" type="submit"></button>`));
	}

	O color(this O)(string value){ this.classes("btn-"~value); return cast(O)this; }
	unittest {
		assert(Assert(BS5ButtonSubmit.color("primary"),`<button class="btn btn-primary" type="submit"></button>`));
	}

	// Set button to disabled
	O disabled(this O)(bool mode = true){ if (mode) this.classes("disabled"); return cast(O)this; }
	unittest {
		assert(Assert(BS5ButtonSubmit.disabled(true),`<button class="btn disabled" type="submit"></button>`));
	}

	// Set outline color
	O outline(this O)(string value){ this.classes("btn-outline-"~value); return cast(O)this; }
	unittest {
		assert(Assert(BS5ButtonSubmit.outline("primary"),`<button class="btn btn-outline-primary" type="submit"></button>`));
	}

	O size(this O)(string value){ this.classes("btn-"~value); return cast(O)this; }
	unittest {
		assert(Assert(BS5ButtonSubmit.size("lg"),`<button class="btn btn-lg" type="submit"></button>`));
	}
}
